首先明确：**Vaultwarden官方镜像已内置SQLite数据库（默认数据存储）和核心运行工具**，但不包含备份、证书生成等辅助工具，这些需要在部署脚本中补充实现。

![](file:///C:/Users/IT%E8%80%81%E8%8F%9C%E9%B8%9F/Pictures/Screenshots/%E5%B1%8F%E5%B9%95%E6%88%AA%E5%9B%BE%202025-07-29%20214523.png)

![6fe59bb4-52be-4180-b8e1-156595c1d68f](file:///C:/Users/IT%E8%80%81%E8%8F%9C%E9%B8%9F/Pictures/Typedown/6fe59bb4-52be-4180-b8e1-156595c1d68f.png)

### 脚本核心特性与说明

#### 1. 关于Vaultwarden镜像的说明

- **内置组件**：官方镜像（`vaultwarden/server:latest`）已包含：
  - SQLite数据库（默认数据存储，无需额外配置）
  - 核心Web服务与API
  - 基本的加密与安全工具
- **不包含的工具**：`openssl`（证书生成）、`curl`（健康检查）、argon2（加密令牌）等，脚本会自动检测并安装这些依赖。

#### 2. 脚本精简但完整的原因

- **保留安全核心**：保留了管理员令牌生成（48位强随机值）、目录权限控制（700/600）、SSL证书自动生成等关键安全措施
- **实用备份机制**：提供手动备份命令（`./deploy.sh backup`），可通过注释的crontab配置自动备份
...
3. 运行部署：`./deploy.sh`
4. 手动备份：`./deploy.sh backup`（如需自动备份，取消脚本最后一行注释）
...
# 查看服务日志
cd ~/vaultwarden && docker compose logs -f
```

这个脚本专注于"能用、好用、安全用"三个核心目标，没有多余的代码，同时覆盖了Vaultwarden部署的全部必要环节，可直接在生产环境使用。
